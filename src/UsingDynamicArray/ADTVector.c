...
// A VectorNode is a Pointer to this struct. (struct only contains one element,
// so we'd go ahead and avoid it, but it makes the code simpler)
struct vector_node {
...
struct vector {
  ...
  int capacity;     // How much space we have reserved (the size of the array).
                    // Always capacity >= size, but we may have
...
void vector_set_at(Vector vec, int pos, void *value) {
  ...
  // If there is a destroy_value function, call it for the element being
  // replaced
...
void vector_insert_last(Vector vec, void *value) {
  // We increase the table (if necessary) so that it can hold at least size
  // elements Double the capacity each time (important for complexity!)
...
void *vector_node_value(VectorNode node) { return node->value; }
