...
void update_pokeball(Object pokeball, KeyState keys, float speed_factor,
                     int score) {
  pokeball->rect.x += keys->right ? 6 : keys->left ? 0.8 : 4;

  if (pokeball->vert_mov == JUMPING) {
    pokeball->rect.y -= (pokeball->vert_speed *
                         speed_factor); // increase upwards => decrease in y
    pokeball->vert_speed *= 0.85;

    if (pokeball->vert_speed <= 0.5)
      pokeball->vert_mov = FALLING;

    // if he has reached the maximum possible height when jumping, the Pokeball
    // must fall
    ...
    pokeball->rect.y += (pokeball->vert_speed *
                         speed_factor); // decrease downwards => increase in y
  ...
  pokeball->pokeball = score <= 20   ? CLASSIC_POKEBALL
                       : score <= 40 ? GREAT_BALL
                       : score <= 99 ? ULTRABALL
                                     : MASTERBALL;
}
