...
Texture staryu_img, starmie_img, bulbasaur_img, charmander_img, picachu_img,
    gengar_img;
Texture torterra_img, eevee_img, snorlax_img, squirtle_img, lucario_img,
    hypno_img, wobbuffet_img;
...
static Texture2D draw_pokemon(Pokemon pokemon) {
  return pokemon == STARYU       ? staryu_img
         : pokemon == STARMIE    ? starmie_img
         : pokemon == BULBASAUR  ? bulbasaur_img
         : pokemon == CHARMANDER ? charmander_img
         : pokemon == PICACHU    ? picachu_img
         : pokemon == ZEKROM     ? zekrom_img
         : pokemon == RESHIRAM   ? reshiram_img
         : pokemon == ARCEUS     ? arceus_img
         : pokemon == REYQUAZA   ? reyquaza_img
         : pokemon == SNORLAX    ? snorlax_img
         : pokemon == EEVEE      ? eevee_img
         : pokemon == TORTERRA   ? torterra_img
         : pokemon == SQUIRTLE   ? squirtle_img
         : pokemon == GENGAR     ? gengar_img
         : pokemon == MEWTWO     ? mewtwo_img
         : pokemon == HYPNO      ? hypno_img
         : pokemon == LUCARIO    ? lucario_img
         : pokemon == AZUMARILL  ? azumarill_img
         : pokemon == WOBBUFFET  ? wobbuffet_img
                                 : gyarados_img;
}

static Texture draw_pokeball(StateInfo info) {
  return info->ball->pokeball == CLASSIC_POKEBALL ? pokeball_img
         : info->ball->pokeball == GREAT_BALL     ? greatball_img
         : info->ball->pokeball == ULTRABALL      ? ultraball_img
                                                  : masterball_img;
}
...
void interface_draw_frame(State state) {
  ...
  objs = state_objects(state, info->ball->rect.x - SCREEN_WIDTH,
                       info->ball->rect.x + SCREEN_WIDTH);

  Object obj;
  for (ListNode node = list_first(objs); node != LIST_EOF;
       node = list_next(node)) {
    obj = list_node_value(node);

    obj->type == PLATFORM
        ? DrawRectangle(obj->rect.x + x_offset - 450, obj->rect.y,
                        obj->rect.width, obj->rect.height,
                        obj->unstable ? RED : WHITE)
        : DrawTexture(draw_pokemon(obj->pokemon), obj->rect.x + x_offset - 450,
                      obj->rect.y, WHITE);
  }
  ...
  if (!info->playing) {
    DrawText("PRESS [ENTER] TO CATCH MORE POKEMON",
             GetScreenWidth() / 2 -
                 MeasureText("PRESS [ENTER] TO CATCH MORE POKEMON", 20) / 2,
