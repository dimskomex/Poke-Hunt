...
ListNode list_last(List list) {
  // Be careful, if the list is empty last points to the dummy, but we want to
  // return NULL, not the dummy!
...
ListNode list_previous(List list, ListNode node) {
  for (ListNode find_node = list->dummy->next; find_node != LIST_EOF;
